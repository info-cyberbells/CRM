  ...
  try {
    ...
    if (token) {
      ...
      if (userRole === "Sale") {
        ...
        // Cases created today
        const todayCases = saleCases.filter(
          (c) =>
        ...
        const monthlyCases = saleCases.filter(
        ...
        const prevMonthlyCases = saleCases.filter(
          (c) =>
            new Date(c.createdAt) >= startOfPrevMonth &&
            new Date(c.createdAt) <= endOfPrevMonth,
        );

        const todaySales = todayCases.reduce((sum, c) => sum + c.saleAmount, 0);
        const monthlySales = monthlyCases.reduce(
          (sum, c) => sum + c.saleAmount,
          0,
        );
        const prevMonthlySales = prevMonthlyCases.reduce(
          (sum, c) => sum + c.saleAmount,
          0,
        );
        ...
        return res.json({
          ...
          casesGrowth:
            prevMonthlyCases.length > 0
              ? (
                  ((monthlyCases.length - prevMonthlyCases.length) /
                    prevMonthlyCases.length) *
                  100
                ).toFixed(2)
              : 0,
          salesGrowth:
            prevMonthlySales > 0
              ? (
                  ((monthlySales - prevMonthlySales) / prevMonthlySales) *
                  100
                ).toFixed(2)
              : 0,
